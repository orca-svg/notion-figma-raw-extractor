    ...
    <main className="tools-guide">
      <section className="tools-hero">
        ...
        <div className="tools-hero-copy">
          <p>개인 토큰은 채널 하나를 이 PC에서 직접 읽고, 관리자 Export는 승인된 전체 대화를 일괄 반입하며, Slack MCP는 인증 사용자가 접근할 수 있는 채널의 최신 상태를 확인합니다.</p>
          <dl className="tools-counts"><div><dt>토큰 연결</dt><dd>{status.web?.connected ? status.web.teamName ?? "연결됨" : "대기"}</dd></div><div><dt>MCP 연결</dt><dd>{status.connected ? "연결됨" : "대기"}</dd></div><div><dt>확인 Tool</dt><dd>{status.tools?.length ?? "—"}</dd></div></dl>
        </div>
      </section>
      <div className="tool-groups">
        <section className="tool-group read"><header><div><span>개인 토큰 · 채널 하나</span><b>DIRECT</b></div><p>slack.com을 직접 호출해 메시지·스레드·작성자를 끝까지 읽습니다. 중간 서버가 없습니다.</p></header><div className="tool-card-grid">{["conversations.info", "conversations.history", "conversations.replies", "users.info"].map((name) => <article className="tool-card" key={name}><code>{name}</code><h2>{name}</h2><p>Slack Web API를 이 PC에서 직접 호출합니다.</p></article>)}</div></section>
